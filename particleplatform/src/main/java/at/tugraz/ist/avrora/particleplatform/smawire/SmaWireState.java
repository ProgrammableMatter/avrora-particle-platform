package at.tugraz.ist.avrora.particleplatform.smawire;

/**
 * Implementation of a {@link SmaWire} states depending on the attached MosFets
 * and MCU pin states. This class covers the states of IC2B and IC3A or IC2A and
 * IC3B. That means, it coves the input/output of two MosFets placed on
 * different PCBs.
 * 
 * @author Raoul Rubien
 *
 */
class SmaWireState
{
    private boolean tx;
    private boolean rxSwitch;
    private boolean rx;


    public SmaWireState()
    {
        reset();
    }


    public boolean isTx()
    {
        return tx;
    }


    public void setTx(boolean tx)
    {
        this.tx = tx;
    }


    public boolean isRx()
    {
        return rx;
    }


    public void setRx(boolean rx)
    {
        this.rx = rx;
    }


    public boolean isRxSwitch()
    {
        return rxSwitch;
    }


    public void setRxSwitch(boolean rxSwitch)
    {
        this.rxSwitch = rxSwitch;
    }


    public void reset()
    {
        tx = false;
        rxSwitch = false;
        rx = true;
    }
}
